...
export default class FullScreenContainer extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if ( !this._isMediaListEqual(this.state.mediaList, nextProps.mediaList) ) {
        this.photoRefs = [];

        var mediaList = nextProps.mediaList;
        var toIndex = Math.min(this.state.currentIndex, nextProps.mediaList.length-1);
        ...
        this.setState({
          ...
          if (this.state.currentIndex >= 0) {
            this.photoRefs.map(p => p && p.forceUpdate());
            this.openPage(toIndex, false);
          }
        });
    }
  }

  _isMediaListEqual(list1, list2) {
    ...
    for (var i = 0, len1 = list1.length; i < len1; i++) {
      if (list1[i].photo != list2[i].photo) {
        return false;
      }
    }
  ...
  _updatePageIndex(index) {
    this.setState({
      currentIndex: index,
      currentMedia: this.state.mediaList[index],
    }, () => {
      this._triggerPhotoLoad(index);

      const newTitle = `${index + 1} of ${this.state.dataSource.getRowCount()}`;
  ...
  _onNextButtonTapped() {
    ...
    if (nextIndex > this.state.dataSource.getRowCount() - 1) {
      nextIndex = 0;
    }
  ...
  _onPreviousButtonTapped() {
    ...
    if (prevIndex < 0) {
      prevIndex = this.state.dataSource.getRowCount() - 1;
    }
  ...
  _renderScrollableContent() {
    const { dataSource, mediaList } = this.state;
